...
In the limit, private intelligence is necessary. My government-assigned AI representative needs to know everything about me to bargain on my behalf. If I want a user aligned model, I need it to know me intimately. Right now, on my computer I have a section of private memories, and private preferences. While none of what’s in my preferences is yet super valuable, it will increasingly be. Companies would love to have it. Authoritarian governments would love to have it. My preferences, perspective, taste, might well become my moat. I might as well start protecting it now.
...
Maintaining Privacy
As I mentioned in the introduction, I try to entirely use GLM, as it’s a good coding model [FN4] (One small drawback is that it isn’t multi-modal, which I deal with in Tinfoil Extensions > Images)
...
To start, we can use this `@import` syntax so that different agents get different prompts, composed of fragments. Claude does this natively but we have to extend Pi to respect it.
...
I want to prevent Github from mining my data that I so carefully prevented AI labs from getting. Git-crypt comes in handy here! It preserves the git tree, which means it shows diffs locally and works like normal git, but stores encrypted ciphertext on github. Very cool.

There’s one more trick – Claude can just do things. To prevent this I made a little information-guard extension. This prevents agents from committing or pushing in the `~agents/` folder. It also prevents specific agents (Claude) from reading private files, using apple sandboxing.
...
I’m a visual person. It’s nice to see things. I have a little script that shows what is shown to which agent, and what is encrypted. `verify-provenance.js` and `show-prompt.js` create a color-coded list and a stitched together prompt markdown file, respectively.
...
Websearch
Search is absolutely essential. I’m not going to read docs. I use Tinfoil’s websearch because it’s accurate, it gives you the full page, it checks for prompt injections, and of course it’s weakly anonymized with a 2 hop system.
...
This is super useful. The other things it allows the Ai to do are interact with vscode. Right now it can add workspaces, and open/close windows. In the future I can add more things, like looking at what I’ve done recently.

This is enabled by a very simple vscode extension (vscode-hatch) that just exposes the entire vscode API. [FN7]

The extension limits the pi agent to only call certain things, though it can get around this easily by just calling the vscode extension api with bash, and I couldn’t figure out how to fix this with apple sandboxing.

Thinking signature fix

A very simple quality of life improvement: the Pi harness drops pure thinking responses returned from GLM5.2. GLM loves to think so this is important. I made this as an extension in case I update pi (`~/.pi/agent/extensions/thinking-signature-backfill.ts`) but it might be worth actually fixing it in Pi.

TODO: Did this fix it for abort?
...
FN1: GLM5.2 served by Tinfoil, a private AI provider (that I work at :)).
FN2: With worse interfaces in some cases, and better ones in others
FN3: Miscellaneous extensions also contains important fixes and is worth not skipping.
...
FN7: Note that this is likely very insecure to run as it just exposes everything to my computer (though I’m not too worried about something breaking my VSCode).
